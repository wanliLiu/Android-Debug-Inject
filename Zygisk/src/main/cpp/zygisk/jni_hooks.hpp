...
static void do_hook_zygote(JNIEnv *env) {
    ...
    //在这个类中遍历nativeForkAndSpecialize_methods 数组对应的方法和签名匹配的方法，没有找的方法fnPtr 便会变成null，找到的方法会把函数artmthode的native函数指针写入
    //同时会在注册一次Zygote 中的函数，将 对应的含顺替换掉
    hookJniNativeMethods(env, clz, nativeForkAndSpecialize_methods.data(), nativeForkAndSpecialize_methods.size());
    // 上个函数替换完成以后，将方法就指针备份出来，然后进行还原调用
