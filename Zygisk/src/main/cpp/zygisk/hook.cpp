...
#endif
//
//        void *handle = load_so_by_fd(fd);
//        if(handle != nullptr){
//            void *  fn = custom_dlsym(handle,"zygisk_module_entry");
//            if(fn != nullptr){
//                modules.emplace_back(i, handle, fn);
//            }
//        } else{
//            LOGE("load_so_by_fd failed");
//        }
            if (void* handle = DlopenMem(fd, RTLD_NOW); void* entry = handle ? dlsym(handle, "zygisk_module_entry") : nullptr) {
        ...
        for (auto &m : modules) {
            ...
            if (flags[APP_SPECIALIZE]) {
                LOGD("preAppSpecialize start");
                m.preAppSpecialize(args.app);
                LOGD("preAppSpecialize end");
            } else if (flags[SERVER_FORK_AND_SPECIALIZE]) {
                LOGD("preServerSpecialize start");
                m.preServerSpecialize(args.server);
                LOGD("preServerSpecialize end");
